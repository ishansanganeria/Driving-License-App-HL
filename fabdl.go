...
type readUserDetailsRTO struct {
	First_Name string `json:"firstname"`
	Last_Name  string `json:"lastname"`
	UIDNo      string `json:"uid"`
	Age        string `json:"age"`
}
type readUserDetailsCop struct {
	First_Name      string `json:"firstname"`
	Last_Name       string `json:"lastname"`
	UIDNo           string `json:"uid"`
	Age             string `json:"age"`
...
func (t *SimpleChainCode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "readUserRTO" { //FETCH USER DATA FOR RTO
		return t.readUserRTO(stub, args)
	} else if function == "readUserCop" { //FETCH USER DATA FOR COP
		return t.readUserCop(stub, args)
	}
...
func (t *SimpleChainCode) readUserRTO(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	uid := args[0]
	fmt.Println("- Reading details for user ", uid)
	...
	var readData readUserDetailsRTO

	readData.First_Name = baseData.BasicData_1.First_Name
	...
	dataJSONasBytes, err := json.Marshal(readData)
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (t *SimpleChainCode) readUserCop(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	var readData readUserDetailsCop

	readData.First_Name = baseData.BasicData_1.First_Name
	readData.Last_Name = baseData.BasicData_1.Last_Name
	readData.UIDNo = baseData.BasicData_1.UIDNo
	readData.Age = baseData.BasicData_1.Age
	readData.BloodGroup = baseData.BasicData_2.BloodGroup
	readData.EmergencyNumber = baseData.BasicData_2.EmergencyNumber
	dataJSONasBytes, err := json.Marshal(readData)
	if err != nil {
		return shim.Error(err.Error())
	}
