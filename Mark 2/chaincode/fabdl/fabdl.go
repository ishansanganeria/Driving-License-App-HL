...
func (t *SimpleChainCode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "AadharApply" { //Add ENtries of baseData2
		return t.AadharApply(stub, args)
	} else if function == "ApproveApplication" { //Add ENtries of baseData2
...
func (t *SimpleChainCode) CreateUserAccount(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	baseData.BasicData_1.ContactNumber = id
...
func (t *SimpleChainCode) AadharApply(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	date := args[1]
	time := args[2]
	filenumber := contact_number

	dataAsBytes, err := stub.GetState(contact_number)
	if err != nil {
	...
	err = stub.PutState(contact_number, dataJSONasBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
...
func (t *SimpleChainCode) ApproveApplication(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	date := args[1]
	time := args[2]
	aadharnumber := string(contact_number) + "A"
	var baseData Aadhar
	err = json.Unmarshal(dataAsBytes, &baseData) //unmarshal it aka JSON.parse()
	if err != nil {
		return shim.Error(err.Error())
	...
	baseData.AadharData.DateOfIssue = date
	baseData.AadharData.AadharNumber = aadharnumber

	var filestatus FileStatusInfo
	filestatus.Status = "Your Application has been granted. Your License number is " + aadharnumber + ". It'll be dispatched shortly"
	filestatus.Date = date
	filestatus.Time = time

	baseData.AadharData.FileStatus = filestatus
...
func (t *SimpleChainCode) ApplyDL(stub shim.ChaincodeStubInterface, args []string) pb.Response {
...
/*
		func (t *SimpleChainCode) ApplyDL(stub shim.ChaincodeStubInterface, args []string) pb.Response {
			...
			contact_number := args[0]
			dataAsBytes, err := stub.GetState(contact_number)
			// dataAsBytes1, err := stub.GetState(contact_number)
			if err != nil {
				return shim.Error("Failed to get user details: " + err.Error())
			} else if dataAsBytes == nil {
				return shim.Error("This user doesn't exist: " + contact_number)
			...
			DLData.ID = contact_number
			DLData.BasicData_1.First_Name = baseData.BasicData_1.First_Name
			DLData.BasicData_1.Last_Name = baseData.BasicData_1.Last_Name
			// = baseData.BasicData_2.BirthPlace
			// = baseData.BasicData_2.Nationality
			// = baseData.BasicData_2.EmergencyNumber
			// = baseData.BasicData_2.BloodGroup

			dataJSONasBytes1, err := json.Marshal(DLData)
			if err != nil {
				return shim.Error(err.Error())
			}

			err = stub.PutState(contact_number, dataJSONasBytes1)
			if err != nil {
				return shim.Error(err.Error())
			}
		...
		func (t *SimpleChainCode) AadharApply(stub shim.ChaincodeStubInterface, args []string) pb.Response {
			...
			date := args[2]
			time := args[3]
			Aaadharnumber := string(args[1][0]) + uid
